...
Como mencionamos en el apartado anterior, proximidad es una generalización de similaridad y disimilaridad. La función disimilaridad, también conocida como función de distancia, en un conjunto de datos X, es definida para satisfacer las siguientes condiciones. Estas condiciones son las utilizadas por \cite{xu2008clustering} y de relevancia en el presente trabajo:
...
donde \(i \leq j,j \leq n\). Los símbolos \(\overrightarrow{D_i}\) y \(\overrightarrow{D_j}\) son documentos en forma de vectores y \(d_c\) es la distancia entre ellos.
